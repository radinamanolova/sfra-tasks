...
server.replace('PlaceOrder', server.middleware.https, function (req, res, next) {
    ...
    if (order.getCustomerEmail()) {
        // add exclusive products to order confirmation email
        let ContentMgr = require('dw/content/ContentMgr');
        let content = ContentMgr.getContent('email-exclusive-products');
        let ProductMgr = require('dw/catalog/ProductMgr');
        let productsIDs = content.custom.body.markup.split("!PRODUCT=");

        function getEmailHeaderText(text) {
            return text.replace(/(<([^>]+)>)/gi, "");
        }

        let emailHeader = productsIDs[0];
        let emailHeaderText = getEmailHeaderText(emailHeader);

        let products = [];
        for (let index = 0; index < productsIDs.length; index++) {
            let product = ProductMgr.getProduct(productsIDs[index]);
            products.push(product);
        }
        COHelpers.sendConfirmationEmailCustom(order, req.locale.id, products, emailHeaderText);
